...
6. **Mobile Accessibility**: The upload flow must include a visible, tactile "Browse Photos" button linked to a hidden file input to ensure functionality on iOS devices, where native drag-and-drop is limited. The review grid and modals must be fully responsive (stacking vertically on mobile, horizontally on desktop) to allow on-the-go triage.
