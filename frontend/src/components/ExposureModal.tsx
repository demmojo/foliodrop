...
export default function ExposureModal({ photo, isOpen, onClose }: ExposureModalProps) {
  ...
  return (
    <div 
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 animate-in fade-in duration-300 dark"
      role="dialog"
      aria-modal="true"
    ...
    >
      <div 
        ref={modalRef}
        className="w-full h-full flex flex-col bg-background text-foreground animate-in zoom-in-95 duration-300"
      >
        {/* Header - Pro Utility aesthetic */}
        <div className="flex-none flex flex-col sm:flex-row items-start sm:items-center justify-between px-4 sm:px-6 py-3 bg-surface border-b border-border z-10 gap-3 sm:gap-0">
          ...
          <button 
            onClick={onClose}
            className="absolute sm:relative top-3 right-4 sm:top-auto sm:right-auto flex items-center gap-2 px-3 py-1.5 rounded-sm text-muted hover:text-foreground hover:bg-border transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-muted group"
            aria-label="Close modal"
          >
            <span className="font-mono text-[10px] uppercase tracking-widest">{t('close') || 'Close'}</span>
            <kbd className="hidden sm:inline-flex items-center justify-center font-sans text-[10px] bg-border px-1.5 py-0.5 rounded border border-border group-hover:bg-muted/20 transition-colors">{t('esc')}</kbd>
          </button>
        </div>
        ...
        <div className="flex-1 flex min-h-0 relative">
          ...
          <div className="w-72 bg-surface flex flex-col border-l border-border shrink-0 overflow-y-auto z-10 shadow-2xl">
             <div className="p-4 border-b border-border">
               <h3 className="text-[10px] font-mono uppercase tracking-widest text-foreground">HITL Editor</h3>
               <p className="text-[10px] text-muted mt-1">Adjust parameters. [A] to Approve, [R] to Reject.</p>
             </div>
             ...
             <div className="p-4 flex flex-col">
               {photo.parameters ? (
                 ...
                 <div className="text-xs text-muted text-center py-10">No VLM parameters available for this image.</div>
               )}
             </div>
          </div>
        </div>
        ...
        {/* Pro Utility Footer */}
        <div className="flex-none px-4 sm:px-6 py-2 bg-surface border-t border-border z-10 flex justify-between items-center overflow-x-auto whitespace-nowrap hide-scrollbar">
